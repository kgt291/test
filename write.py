# -*- coding: cp949 -*-
def write(a, num, chance, cuv, stop, tv):  
  for i in range(0, int(a)):
    if stop[i] == 0: 
      ...
      f.write(str(chance)+"         ! lcurs, number of current changes\n")
      totaltime = 0.0
      for ch in range(chance):  
        cu=cuv[ch][i-1]
        tt=tv[ch][i-1]
        mc="1"
        vcutlo="2.0"
        vcuthi="4.4"
        f.write(str(cu)+"d0  "+ str(tt) +"d0  "+mc+"  "+vcutlo+"d0  "+vcuthi+"d0\n")
